...
def split_dataset_tcga_brca(file_path, conf):
    # Load the provided CSV (adjust path if needed)
    csv_path = './dataset_csv/brca_idc_ilc_pure.csv'  # Or wherever you've saved it
    slide_info = pd.read_csv(csv_path).set_index('slide_id')
    
    # Map labels to integers (assuming binary classification: IDC=0, ILC=1)
    label_map = {'IDC': 0, 'ILC': 1}
    slide_info['label'] = slide_info['label'].map(label_map)
    
    # Group by patient_id to avoid data leakage (splits should respect patient boundaries)
    patient_groups = slide_info.groupby('patient_id')
    patients = list(patient_groups.groups.keys())
    labels_per_patient = [patient_groups.get_group(p)['label'].iloc[0] for p in patients]  # Assume one label per patient
    
    # Stratified split on patients following the paper's 65:10:25 (train:val:test) ratios
    from sklearn.model_selection import train_test_split
    
    # First, split into train+val (75%) and test (25%)
    train_val_patients, test_patients, train_val_labels, test_labels = train_test_split(
        patients, labels_per_patient, test_size=0.25, stratify=labels_per_patient, random_state=conf.seed
    )
    
    # Then, from train+val, split into train (65/75 ≈ 86.67%) and val (10/75 ≈ 13.33%)
    train_patients, val_patients, train_labels, val_labels = train_test_split(
        train_val_patients, train_val_labels, test_size=10/75, stratify=train_val_labels, random_state=conf.seed
    )
    
    # Map patients back to slides
    def get_slides_for_patients(patients_list):
        return [slide for patient in patients_list for slide in patient_groups.get_group(patient).index]
    
    train_names = get_slides_for_patients(train_patients)
    val_names = get_slides_for_patients(val_patients)
    test_names = get_slides_for_patients(test_patients)
    ...
    for names, split in [(train_names, train_split), (val_names, val_split), (test_names, test_split)]:
        for slide_id in names:
            slide = h5_data[slide_id]
            # Always use the mapped label from CSV to ensure it's an integer
            label = slide_info.loc[slide_id]['label']
            feat = slide['feat'][:]
            coords = slide['coords'][:]
    ...
    for names, split in [(train_names, train_split), (val_names, val_split), (test_names, test_split)]:
        for slide_id in names:
            slide = h5_data[slide_id]
            label = slide.attrs['label']  # Use HDF5 attr if available, else fallback to CSV
            if 'label' not in slide.attrs:  # Fallback if HDF5 doesn't have label
                label = slide_info.loc[slide_id]['label']
            feat = slide['feat'][:]
            coords = slide['coords'][:]
            split[slide_id] = {'input': feat, 'coords': coords, 'label': label}
