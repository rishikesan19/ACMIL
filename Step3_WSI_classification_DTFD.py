...
def train_one_epoch(
    ...
    for data_it, data in enumerate(
        ...
        adjust_learning_rate(
            optimizer0, epoch + data_it / len(data_loader), conf
        )
        adjust_learning_rate(
            optimizer1, epoch + data_it / len(data_loader), conf
        )
        ...
        for tindex in index_chunk_list:
            ...
            subFeat_tensor = torch.index_select(
                tfeat_tensor, dim=0, index=tindex
            )
            ...
            max_inst_feat = tmidFeat.index_select(
                dim=0, index=topk_idx_max
            )
        ...
        torch.nn.utils.clip_grad_norm_(
            dimReduction.parameters(), conf.grad_clipping
        )
        torch.nn.utils.clip_grad_norm_(
            attention.parameters(), conf.grad_clipping
        )
        torch.nn.utils.clip_grad_norm_(
            classifier.parameters(), conf.grad_clipping
        )
        optimizer0.step()
        ...
        loss1.backward()
        torch.nn.utils.clip_grad_norm_(
            UClassifier.parameters(), conf.grad_clipping
        )
        optimizer1.step()
...
def evaluate(
    ...
    AUROC_metric = torchmetrics.AUROC(
        num_classes=conf.n_class, task='multiclass'
    ).to(device)
    ...
    print(
        '* Acc@1 {top1.global_avg:.3f} loss {losses.global_avg:.3f} auroc {AUROC:.3f} f1_score {F1:.3f}'
        .format(
...
def main():
    ...
    auto_name = (
        f"{conf.dataset}-{conf.pretrain}-dtfdmil-"
    ...
    wandb.init(
        ...
        project=getattr(args, 'wandb_project', 'ADR'),
        name=conf.exp_name,
    ...
    attCls = Attention_with_Classifier(
        L=conf.D_inner, num_cls=conf.n_class, droprate=0
    ).to(device)
    ...
    optimizer_adam0 = torch.optim.Adam(
        trainable_parameters, lr=conf.lr, weight_decay=conf.wd
    )
    optimizer_adam1 = torch.optim.Adam(
        attCls.parameters(), lr=conf.lr, weight_decay=conf.wd
    )
    ...
    for epoch in range(conf.train_epoch):
        ...
        if val_f1 + val_auc > best_state['val_f1'] + best_state['val_auc']:
            ...
            save_model(
                conf=conf,
                model=classifier,
                optimizer=optimizer_adam0,
                epoch=epoch,
                save_path=os.path.join(ckpt_dir, 'checkpoint-best_classifier.pth')
            )
            save_model(
                conf=conf,
                model=attention,
                optimizer=optimizer_adam0,
                epoch=epoch,
                save_path=os.path.join(ckpt_dir, 'checkpoint-best_attention.pth')
            )
            save_model(
                conf=conf,
                model=dimReduction,
                optimizer=optimizer_adam0,
                epoch=epoch,
                save_path=os.path.join(ckpt_dir, 'checkpoint-best_dimReduction.pth')
            )
            save_model(
                conf=conf,
                model=attCls,
                ...
                epoch=epoch,
                save_path=os.path.join(ckpt_dir, 'checkpoint-best_attCls.pth')
            )
        print('\n')
    save_model(
        conf=conf,
        ...
        optimizer=optimizer_adam0,
        epoch=epoch,
        save_path=os.path.join(ckpt_dir, 'checkpoint-last_classifier.pth')
    )
    save_model(
        conf=conf,
        model=attention,
        optimizer=optimizer_adam0,
        epoch=epoch,
        save_path=os.path.join(ckpt_dir, 'checkpoint-last_attention.pth')
    )
    save_model(
        conf=conf,
        model=dimReduction,
        optimizer=optimizer_adam0,
        epoch=epoch,
        save_path=os.path.join(ckpt_dir, 'checkpoint-last_dimReduction.pth')
    )
    save_model(
        conf=conf,
        model=attCls,
        optimizer=optimizer_adam1,
        epoch=epoch,
        save_path=os.path.join(ckpt_dir, 'checkpoint-last_attCls.pth')
    )
